...
**Item 5 — operationalized liveness** (`liveness-store.mjs`, `liveness-sweep.mjs`, sync + pipeline-grid)
- Before: `check-liveness.mjs` existed but was never run on cadence; liveness status wasn't shown per-job; dead listings could sit in the actionable queue indefinitely.
- After: `data/liveness.jsonl` holds per-URL verdicts (live/dead/unknown + last-verified). `liveness-store.mjs` seeds dead reliably from existing signals (scan-history `skipped_expired` + pipeline `[!]` markers); `liveness-sweep.mjs` re-verifies stale pending URLs on demand via the existing Playwright check (sequential, degrades gracefully if Playwright is absent — kept OUT of the page-load path). Sync projects `liveness` + `last_verified`; the canonical store also yields a `reposted` flag (sightings spanning >14d). UI: a "closed"/"unverified" chip + a "reposted" chip on cards; **dead listings auto-hidden from the default view with a reversible "N closed hidden — show" toggle** (chosen behavior — pipeline.md is never rewritten).
- Verified end-to-end in-browser: a real sweep marked a FINRA listing dead → it vanished from the default view → toggle revealed it with a red "closed" chip. Screenshot `roadmap5-liveness.png`. 135 tests.
- Operational note: run `node liveness-sweep.mjs` on a cadence (or wire a scheduled task) — deliberately not auto-run, and never auto-scheduled without the user's say.

**Item 6 — freshness semantics** (`web/lib/sync.ts`, `web/components/pipeline-grid.tsx`)
...
**Item 8 — explainable fit/winnability** (`web/lib/fit.ts`, drawer FitPanel) — DETERMINISTIC, no API cost
- Before: a single opaque match-score number, no breakdown, no concerns, no degree-flexibility signal.
